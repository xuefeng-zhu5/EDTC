...
from lib.models.uavtrack.score_decoder import ScoreDecoder
...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.bottleneck = nn.Conv2d(dim_in * 2, dim_in, kernel_size=1)
    ...
    def forward_conv_test(self, x, s_h, s_w):
        ...
        if self.conv_proj_k is not None:
            k_s = self.conv_proj_k(search)
        else:
            k_s = rearrange(search, 'b c h w -> b (h w) c').contiguous()
        k = torch.cat([self.t_k, self.ot_k, k_s], dim=1)

        if self.conv_proj_v is not None:
            v_s = self.conv_proj_v(search)
        else:
            v_s = rearrange(search, 'b c h w -> b (h w) c').contiguous()
        v = torch.cat([self.t_v, self.ot_v, v_s], dim=1)
    ...
    def forward(self, x, t_h, t_w, s_h, s_w):
        ...
        # k_t, k_ot, A = torch.split(k, [t_h*t_w//4, t_h*t_w//4, s_h*s_w//4], dim=2)
        # v_t, v_ot, v_s = torch.split(v, [t_h * t_w // 4, t_h * t_w // 4, s_h * s_w // 4], dim=2)
        ...
        # search region attention # search region(Q), [template, search region](KV)
        attn_score = torch.einsum('bhlk,bhtk->bhlt', [q_s, k]) * self.scale
        attn = F.softmax(attn_score, dim=-1)
        attn = self.attn_drop(attn)
        x_st = torch.einsum('bhlt,bhtv->bhlv', [attn, v])
        x_st = rearrange(x_st, 'b h t d -> b t (h d)')

        # search region attention # search region(Q, K, V)
        ...
        # concat x_st and x_s across channel dimension
        x_sts = torch.cat([x_st, x_s], dim=2)
        x_sts = rearrange(x_sts, 'b (h w) c -> b c h w', h=s_h, w=s_w).contiguous()
        x_sts = self.bottleneck(x_sts)
        x_sts = rearrange(x_sts, 'b c h w -> b (h w) c').contiguous()

        x = torch.cat([x_mt, x_sts], dim=1)
...
def get_uavtrack_eh_model(config, **kwargs):
...
class UAVTrackEH(nn.Module):
    """ uav tracking branch with evidential head, whcih jointly perform feature extraction and interaction. """
    ...
    def forward_head(self, search, template, run_score_head=True, gt_bboxes=None):
        ...
        if run_score_head:
            ...
            out_dict.update({'pred_scores': self.score_branch(search, template, gt_bboxes)})
...
def build_uavtrack_eh(cfg, settings=None, train=True):
    backbone = get_uavtrack_eh_model(cfg)  # backbone without positional encoding and attention mask
    box_head = build_box_head(cfg)  # a simple corner head
    score_branch = ScoreDecoder(pool_size=4, hidden_dim=cfg.MODEL.HIDDEN_DIM, num_heads=cfg.MODEL.HIDDEN_DIM//64)  # evidential head
    model = UAVTrackEH(
        backbone,
        box_head,
        score_branch,
    ...
    if cfg.MODEL.PRETRAINED_STAGE1 and train:
        try:
            ...
            if is_main_process():
                print("missing keys:", missing_keys)
                print("unexpected keys:", unexpected_keys)
                print("Loading pretrained uavtrack weights done.")
        except:
            print("Warning: Pretrained uavtrack weights are not loaded")
