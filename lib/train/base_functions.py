...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, TNL2k, AntiUAV
...
def update_settings(settings, cfg):
    ...
    settings.use_uncertainty = getattr(cfg.TRAIN, 'USE_UNCERTAINTY', False)
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "COCO17", "VID", "TRACKINGNET",
                        "TNL2k", "AntiUAV_train", "AntiUAV_val"]
        if name == "LASOT":
        ...
        if name == "AntiUAV_train":
            datasets.append(AntiUAV(settings.env.antiuav_dir, split='train', image_loader=image_loader))
        if name == "AntiUAV_val":
            datasets.append(AntiUAV(settings.env.antiuav_dir, split='val', image_loader=image_loader))
...
def build_dataloaders(cfg, settings):
    ...
    data_processing_train = processing.UAVTrackProcessing(search_area_factor=search_area_factor,
                                                           output_sz=output_sz,
    ...
    data_processing_val = processing.UAVTrackProcessing(search_area_factor=search_area_factor,
                                                         output_sz=output_sz,
                                                         center_jitter_factor=settings.center_jitter_factor,
                                                         scale_jitter_factor=settings.scale_jitter_factor,
                                                         mode='sequence',
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_score:
        ...
        print("Freeze Stage0 of UAVTracking backbone. Learnable parameters are shown below.")
        for n, p in net.named_parameters():
        ...
        print("Freeze the first 6 layers of UAVTracking backbone. Learnable parameters are shown below.")
        for n, p in net.named_parameters():
